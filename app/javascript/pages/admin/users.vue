  ...
  div
    v-container(grid-list-md)
      v-dialog(v-model="dialog", max-width="500px")
        v-btn(slot="activator", color="primary", dark).mb-2 New Item
        v-card
          v-card-title
      ...
      v-data-table(:headers="headers", :items="desserts", hide-actions).elevation-1
        template(slot="items", slot-scope="props")
          ...
          td(layout px-0).justify-center
            v-btn(icon, @click="editItem(props.item)").mx-0
              v-icon(color="teal") edit
            v-btn(icon, @click="deleteItem(props.item)").mx-0
